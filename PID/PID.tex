...
\section{補足}
一般に、式(\ref{eq:PID_practice5})に示すようなPIDの制御式をそのまま実装することは難しい場合がある。例えば、本評価のように、離散的な時間間隔で制御を行う場合は、微分や積分を計算できない。これは実装依存の問題になるのだが、微分や積分が計算できない場合はそれらの近似を用いる手法があり、本評価でも微分、積分を近似している。
% \begin{eqnarray}
